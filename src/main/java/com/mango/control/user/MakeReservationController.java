...
//                System.out.println(room_id + "######");
                room_id_and_time_id.put(room_id.concat(time_id), new HashMap<String,String>() {{
                    put(room_id, time_id);
                }});
            }
    ...
    public String reservation_check(HttpServletRequest request, Model model) {
        ...
        // First, check if the student is on the blacklist
        BlackList student = blackListService.getBlackedStudentById(loginUser.getS_id());
        System.out.println(student);

        if (student != null) {

            model.addAttribute("msg", "You are on the blacklist and cannot make a reservation!");
            return "confirm_reservation";
        } else {
            // If not on the blacklist, proceed to the next check
            // Check if the student has canceled reservations more than three times in a week, by querying the cancellation records
            if (studentService.isThreeTimesCanceledOfWeekById(loginUser.getS_id()) == true) {
                model.addAttribute("msg", "You have canceled reservations more than three times in a week, unable to make a reservation!");
                return "confirm_reservation";
            }
        }
